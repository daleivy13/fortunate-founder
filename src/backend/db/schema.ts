import {
  ...
  timestamp, serial, pgEnum, unique,
} from "drizzle-orm/pg-core";
import { relations } from "drizzle-orm";

// ── Enums ──────────────────────────────────────────────────────────────────────
export const planEnum          = pgEnum("plan",          ["starter", "solo", "growth", "enterprise"]);
...
export const chemistryReadings = pgTable("chemistry_readings", {
  ...
  aiDosageRecommendation: text("ai_dosage_recommendation"),
  notes:                  text("notes"),
...
export const mileageLogs = pgTable("mileage_logs", {
  ...
  deductionRate: real("deduction_rate").default(0.67),
...
export const poolTaskHistory = pgTable("pool_task_history", {
  id:          serial("id").primaryKey(),
  taskId:      integer("task_id").references(() => poolTasks.id).notNull(),
  poolId:      integer("pool_id").references(() => pools.id).notNull(),
  completedBy: text("completed_by"),
...
// ── Chemical Inventory ────────────────────────────────────────────────────────
export const chemicalInventory = pgTable("chemical_inventory", {
  id:          serial("id").primaryKey(),
  companyId:   integer("company_id").references(() => companies.id).notNull(),
  name:        text("name").notNull(),
  unit:        text("unit").notNull(),
  currentQty:  real("current_qty").default(0),
  minQty:      real("min_qty").default(0),
...
export const clientRatings = pgTable("client_ratings", {
  id:         serial("id").primaryKey(),
  poolId:     integer("pool_id").references(() => pools.id).notNull(),
  reportId:   integer("report_id").references(() => serviceReports.id),
...
export const referrals = pgTable("referrals", {
  ...
  status:       text("status").default("pending"),
  creditAmount: real("credit_amount").default(50),
  createdAt:    timestamp("created_at").defaultNow(),
...
export const homeownerProfiles = pgTable("homeowner_profiles", {
  ...
  subStatus:    text("sub_status").default("free"),
  createdAt:    timestamp("created_at").defaultNow(),
});
